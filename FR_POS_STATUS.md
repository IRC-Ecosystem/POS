...
| --- | --- | --- | --- |
| FR-POS-001 | Kasir dapat membuat invoice dari produk dan jumlah. | Sesuai sebagian besar | Ada `POST /kasir/direct-sale`; kasir memilih produk dan quantity, lalu sistem membuat transaksi/invoice. |
...
| FR-POS-004 | Invoice digital hanya `PAID` jika payment sukses. | Sesuai sebagian | Untuk order konsumen alurnya `pending -> approved -> paid`. Untuk direct sale kasir, transaksi langsung dibuat `paid`, belum melalui state `PENDING_PAYMENT -> PAID`. |
...
| FR | Kekurangan Utama |
| --- | --- |
| FR-POS-001 | Invoice POS sudah ada lewat direct sale, tetapi model state invoice PRD belum sepenuhnya dipakai. |
...
| FR-POS-004 | Direct sale langsung `paid`, belum menunggu payment state terpisah. |
